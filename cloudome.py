...
def return_seg_edge(task: SynapseEdgeTaskPayload) -> tuple[SegmentID, SegmentID]:
    ...
    try:
        ...
        x_min, x_max = (
            max(0, xyz_center[0] - RADIUS),
            min(bounds[0], xyz_center[0] + RADIUS),
        )
        y_min, y_max = (
            max(0, xyz_center[1] - RADIUS),
            min(bounds[1], xyz_center[1] + RADIUS),
        )
        z_min, z_max = (
            max(0, xyz_center[2] - RADIUS),
            min(bounds[2], xyz_center[2] + RADIUS),
        )
        ...
        prepost_mask = synapse_volume[
            x_min:x_max, y_min:y_max, z_min:z_max, 0
        ].squeeze()
        seg_mask = segmentation_volume[
            x_min:x_max, y_min:y_max, z_min:z_max, 0
        ].squeeze()
...
def return_ctc_edges(task: ContactomeEdgeTaskPayload):
    ...
    try:
        ...
        x_min, x_max = (
            max(0, xyz_start[0]),
            min(bounds[0], xyz_start[0] + xyz_radius[0] + 1),
        )
        y_min, y_max = (
            max(0, xyz_start[1]),
            min(bounds[1], xyz_start[1] + xyz_radius[1] + 1),
        )
        z_min, z_max = (
            max(0, xyz_start[2]),
            min(bounds[2], xyz_start[2] + xyz_radius[2] + 1),
        )
        if x_min >= x_max or y_min >= y_max or z_min >= z_max:
            raise ValueError(
                "Slicing range is invalid due to out-of-bounds coordinates."
        ...
        initial_contact_counts = count_contact_voxels(
            seg_mask, segmentation_volume.resolution
        )
...
def return_volume_counts(task: VolumeTaskPayload):
    ...
    try:
        ...
        x_min, x_max = (
            max(0, xyz_start[0]),
            min(bounds[0], xyz_start[0] + xyz_radius[0]),
        )
        y_min, y_max = (
            max(0, xyz_start[1]),
            min(bounds[1], xyz_start[1] + xyz_radius[1]),
        )
        z_min, z_max = (
            max(0, xyz_start[2]),
            min(bounds[2], xyz_start[2] + xyz_radius[2]),
        )
