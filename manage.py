...
def generate_cuboidwise_tasks_for_contactome_or_volume(
    ...
    for i, ((x_start, x_stop), (y_start, y_stop), (z_start, z_stop)) in tqdm(
        enumerate(blocks)
    ):
        if enqueue_limit is not None and i >= enqueue_limit:
            break
...
def parse_arguments():
    ...
    parser.add_argument(
        "--sqs-url",
        type=str,
        default="https://sqs.us-east-1.amazonaws.com/407510763690/CloudomeJobs",
...
def main():
    ...
    if "," in args.mip:
        ...
        try:
            ...
            print(
                f"Error: MIP value '{args.mip}' is not valid. Use a single integer or comma-separated integers."
            )
            exit(1)
