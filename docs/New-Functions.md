...
-   Generators live in `shared_*_utils.py`. They yield `TypedDict` payloads.
-   Examples:
    -   `generate_cuboidwise_tasks` in `shared_cuboid_utils.py` produces `ContactomeEdgeTaskPayload | VolumeTaskPayload` from a segmentation source and block layout.
    -   `generate_centroidwise_tasks` in `shared_synapse_utils.py` reads centroid CSVs and yields `SynapseEdgeTaskPayload`.
...
-   In `database.py`, add a new `TypedDict` for your task, e.g.:
...
## Local Worker Handler (`local_manage.py`)
...
-   If your task needs volume/segmentation windowing, reuse CloudVolume bounds helpers from `cloudome.py` (e.g., `_bbox_min_max`, `return_volume_counts`).

## CLI Wiring

-   Extend `parse_arguments()` in `local_manage.py`:
...
-   Ensure the model table/queue exists (see `manage.py initialize_resources`).
...
1. Payload: add `MyTaskPayload` in `database.py`.
2. Generator: add `generate_mytask_payloads(...)` under `shared_*_utils.py`.
3. Worker: implement `@queueable _process_mytask(...)` in `local_manage.py`.
...
```bash
uv run local_manage.py mytask generate [flags]
uv run local_manage.py worker --dequeue-limit 5
```
...
-   Compute functions live in `cloudome.py` and are callable from both cloud and local paths.
