...
def local_dequeue(sqs_url: str):
    from . import cloudome as cloudome_app

    response = sqs.receive_message(QueueUrl=sqs_url, MaxNumberOfMessages=1)
    if "Messages" in response:
        for message in response["Messages"]:
            cloudome_app.process_queue_job({"Records": [message]}, None)
...
def parse_arguments():
    ...
    initialize_parser = subparsers.add_parser(
        "initialize", help="Provision shared cloud resources"
    )
...
def main():
    ...
    elif args.namespace == "initialize":
